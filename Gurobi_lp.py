...
def main():
    ...
    #Linear constraints
    c4 = opt_model.addConstrs((grb.quicksum(
        grb.quicksum(x_vars[(i, k)] * data['Heats'][i] for i in data['Names']) for k in
        data['Bins']) / num_bins - grb.quicksum(x_vars[(i, j)] * data['Heats'][i] for i in data['Names'])) <= t_vars[j]
                              for j in data['Bins'])
    c5 = opt_model.addConstrs((grb.quicksum(
        grb.quicksum(x_vars[(i, k)] * data['Heats'][i] for i in data['Names']) for k in
        data['Bins']) / num_bins - grb.quicksum(x_vars[(i, j)] * data['Heats'][i] for i in data['Names'])) >= -t_vars[j]
                              for j in data['Bins'])
    ...
    dframe.to_excel('./data2050-27.06_LP.xlsx')
